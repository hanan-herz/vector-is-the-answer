...
weakest (RuleTaker $+12.0$pt on Granite-8B, $+5.2$pt at 0.6B; BoolQ $+3.0$pt
at 0.6B against a $4\times$-budget loop), ties at mid scale, and loses
decisively only in three regimes --- small models on parametric knowledge
(ARC), a frontier MoE on serial deduction, and Mistral-7B on BoolQ. The result survives label-shuffle
...
cells and never loses. The model, in short, knows more than it can say
\citep{orgad2025know,gekhman2025insideout} --- on this task class,
\textbf{the vector already has the answer.} We bound the claim explicitly:
it is \head{}-vs-loop on a frozen backbone, not
...
\noindent\emph{Claim discipline.} The \head{} matches or beats the loop in 15
of 18 cells. The three loop wins are a frontier MoE on serial deduction
(RuleTaker at DeepSeek-V4, $-3.8$pt), Mistral-7B on BoolQ ($-2.2$pt), and
small models on parametric knowledge (ARC, $-10.6 \to -0.7$pt as models
grow). The remaining BoolQ cells are statistical ties --- four of six sit
within $\pm$1.2pt, all non-significant. The contribution is the \emph{map} of
where each readout wins, not a blanket verdict --- and the map is mostly one
color.
...
0.6B, ties at 4B/8B, and trails the few-shot loop by $\sim$1pt (ns) at
Granite-8B and the frontier MoE} (Table~\ref{tab:main}). The one exception is
Mistral-7B, where the loop wins by $2.2$pt ($p=2.6\mathrm{e}{-03}$) --- the
single significant loop win on BoolQ, which we flag as a single-model
exception (\S\ref{sec:discussion}).
...
the fair 8-shot loop at every scale} ($+3$ to $+5$pt; significant at 0.6B and
4B, $p=3.8\mathrm{e}{-03}$ and $5.3\mathrm{e}{-03}$; 8B is a near-tie
...
one clean loop win is DeepSeek-V4 ($-3.8$pt, $p=8.5\mathrm{e}{-03}$), driven
...
are exactly where the loop is weakest (Granite RuleTaker $+12.0$pt; 0.6B
everywhere) --- the \head{} channel saturates before the verbal channel. This
is the internal-knowledge/external-behavior disconnect
\citet{orgad2025know} document and \citet{gekhman2025insideout} formalize as
\emph{hidden knowledge}; we show it has \emph{operational} content: a
producer \head{} on the prompt residual is a drop-in replacement for the
classifier the loop provides. That the \head{} is near-linear
(\S\ref{sec:boolq}) sharpens the point: the answer is a near-linear read of a
vector the model already computes.
...
\item \textbf{Paired tests cover all 18 placement cells and the BoolQ budget
  arm,} but the per-depth loop strata are thin ($n=21$--$51$), and 3 of the 9
  significant placement wins do not survive Benjamini--Hochberg.
...
\item \textbf{Scope}: in-knowledge, closed-set tasks only. No claim transfers
...
representation directly: CCS \citep{burns2022ccs}, pre-CoT belief probes
\citep{cox2026precommitted}, belief-formation timing
...
that head to a fairly-conditioned loop under matched supervision.
...
fit to gold labels, run head-to-head against a fairly-conditioned loop,
matched on supervision budget, information, and scoring, under
format/shuffle/randproj controls --- is the one this paper occupies. Turpin
saw behavior, Cox saw belief, Orgad detected errors and re-ranked samples,
Gekhman formalized hidden knowledge with a verifier on $h(q,a)$, Cho beat a
baseline they showed was weak, Hazenoot beat zero-shot prompting, ReProbe
verified steps to generate better; we compare a one-pass producer and the
loop at producing the right answer, because the weights are open and frozen.
...
make. Trajectory-dynamics probes \citep{damirchi2026tat} read layer-wise
...
the strongest classifier the autoregressive loop provides across the space we
measured. Three loop wins remain: a frontier MoE on serial deduction
(RuleTaker at DeepSeek-V4), Mistral-7B on BoolQ, and a small-model parametric
gap (ARC) that closes with scale; elsewhere the loop's edges are statistical
ties (four of six BoolQ cells within $\pm$1.2pt). The claim is
therefore bounded but strong: on this task class, the vector already has the
answer --- and we map the boundaries where the loop still earns its keep.
